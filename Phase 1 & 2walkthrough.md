...
### Phase 2: Stage 2 Scoring Reordering & Borderline IPC Gating
- **Module:** `core/ai_client.py` (`evaluate_job_match()`)
- **Stage 2 Reordering:** In Zero-API mode, calibrated deterministic factual scoring (Title 0-35, Core Skills 0-45 requiring min 2 core skills, Exp 0-20) executes first, establishing a verified baseline score.
- **Strict Borderline IPC Gating ($40\% \le \text{score} \le 65\%$):**
  - **High-Fit Domain Roles:** Score $\ge 60\%$ (typically $75-100\%$) and qualify instantly without halting the scraper or creating `pending_question.json`.
  - **Blatant Out-of-Domain Roles:** Drop at Stage 1 ($0\%$) without writing `pending_question.json`.
  - **Clear Rejections:** Score $< 40\%$ and resolve immediately as rejected without writing `pending_question.json`.
  - **Borderline Roles ($40-65\%$):** Only roles in the ambiguous window emit a structured `JOB_EVALUATION` payload to `pending_question.json` for cognitive arbitration.
- **Hybrid `MatchResult` Contract:** Extended with `__contains__`, `keys()`, `values()`, and `items()` for full dictionary polymorphism while maintaining complete tuple unpacking (`score, reasoning, matching, missing = result`) and attribute access.
- **Automated Verification:**
  - Validated via dedicated 5-test suite (`TestPhase2JobEvaluation`):
    1. `test_1_match_result_hybrid_contract`: PASSED (tuple unpacking, indexing, attribute access, dict methods).
    2. `test_2_blatant_out_of_domain_drops_stage1_zero_ipc`: PASSED (Stage 1 drop at 0% with zero IPC calls).
    3. `test_3_high_fit_domain_job_qualifies_zero_ipc`: PASSED (High-fit domain job scored 100% and qualified instantly with zero IPC calls).
    4. `test_4_clear_rejection_resolves_zero_ipc`: PASSED (Clear rejection scored < 40% with zero IPC calls).
    5. `test_5_borderline_role_triggers_ipc_evaluation`: PASSED (55% borderline score emitted structured `JOB_EVALUATION` payload to IPC and received arbitrated result).
