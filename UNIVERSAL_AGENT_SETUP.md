...
- **Empirical DOM Invariant vs. Dynamic Runtime Separation:** Decouples invariant layout tags, classes, and attributes from candidate-specific and job-specific runtime data, ensuring resilient scraping across portal releases.
- **Granular SRP Card Telemetry (`04_job_discovery.py`):** Pinpoints and extracts full Search Results Page card data: Designation (`a.title`), Company (`a.comp-name`), Rating (`a.rating span.main-2`), Reviews (`a.review`), Experience (`span.exp-wrap`), Salary (`span.sal-wrap`), Location (`span.loc-wrap`), Description snippet (`span.job-desc`), Skill chips (`ul.tags-gt li.dot-gt.tag-li`), Recency (`span.job-post-day`), and Bookmark icons (`span.save-job-tag`).
- **"Read More" Un-Clamping & Deep JD Ingestion:** Automatically detects and clicks `span.styles_rm-link__RgrMs` (`.customReadMoreLabelClass`), expanding `-webkit-line-clamp: 5` on `div.styles_read-more__TFiRZ` to ingest complete job descriptions (7,200+ characters) covering Job Highlights, Responsibilities, Benefits, Specifications, Education, and Key Skills.
- **Naukri Native Match Score Cognitive Booster:** Scrapes portal-verified match criteria (`div.styles_JDC__match-score__VnjLL` for `Early Applicant`, `Keyskills`, `Location`, `Work Experience`) via state icons (`.ni-icon-check_circle` vs `.ni-icon-crossMatchscore`). Integrates verified signals into `core/ai_client.py` Stage 2 evaluation, granting up to a +10% confidence bonus for matched Keyskills and Work Experience.
- **Cognitive Selective Profile Sync Engine:** 5-step cognitive updating engine (`02_profile_sync_naukri.py`) that audits live portal profiles against ground-truth resumes, generates per-role JSON evaluation cards (`KEEP_EXISTING` vs `UPDATE_REQUIRED` vs `ADD_NEW`) in `output/profile_sync/naukri_cards/`, and performs surgical updates via verified modal selectors (Headline `textarea#resumeHeadlineTxt`, Skills `.suggester-input input`, Employment `form#employmentForm`).
...
2. **Discovery (`core/04_job_discovery.py`):**
   ...
   - Extracts complete SRP card metadata: Designation (`a.title`), Company (`a.comp-name`), Rating & Reviews (`a.rating`, `a.review`), Experience (`span.exp-wrap`), Salary (`span.sal-wrap`), Location (`span.loc-wrap`), Description snippet (`span.job-desc`), and Skill tags (`ul.tags-gt li`).
   - Checks the $O(1)$ persistent ledger (`processed_ledger.json`) to skip previously evaluated jobs.
   - On detail page navigation, automatically un-clamps "Read More" (`span.styles_rm-link__RgrMs`), expanding truncated text from 3.4k to 7.2k+ characters across all JD sections.
   - Scrapes Naukri Native Match Score (`div.styles_JDC__match-score__VnjLL`) for all 4 criteria (`Early Applicant`, `Keyskills`, `Location`, `Work Experience`).
   - Evaluates match scores via two-stage filtering with portal confidence bonuses (+10% for Keyskills + Exp): $\ge 60\%$ qualifies, $< 40\%$ rejects, and $40-65\%$ invokes Antigravity 2.0 IPC arbitration.
   - Persists `naukri_match_score` directly into `job_details.json` and `search_manifest.json`.
