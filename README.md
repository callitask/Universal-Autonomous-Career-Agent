...
*   **Two-Stage Page Navigation Timeout Recovery (Guardrail C32):** Resilient two-stage navigation (`commit` [12s] + `domcontentloaded` [15s]) that gracefully recovers from third-party syndicated gateway timeouts without crashing or leaving zombie tabs.
