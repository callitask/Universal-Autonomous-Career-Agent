...
### Native LinkedIn Easy Apply Modal Automation
- **Class:** `LinkedInApplyHandler` in `core/05_apply_jobs.py`
- **Capabilities:**
  - Detects and scopes interactions to `div.jobs-easy-apply-modal` or `div[data-test-modal]`.
  - Auto-populates phone number from `candidate.phone`.
  - Resolves freeform and numerical questions via `AIClient.answer_screening_question()`.
  - Selects radio options and handles dropdown menus.
  - Automatically attaches the dynamically tailored ATS PDF (`input[type='file']`).
  - Navigates multi-step modal dialogs ("Next", "Review", "Submit application").
  - Discards modal cleanly upon detecting unresolvable mandatory fields.
