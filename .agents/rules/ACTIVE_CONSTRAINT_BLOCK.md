...
# Version: 1.3 | Updated: 2026-09-22 | Loaded: auto (global + workspace)
# Token budget: ~200 tokens. Purposely minimal. Do NOT expand.

## 11 HARD GATES - Verify ALL before proceeding with any action:
...
Career Agent (core/, scripts/, profiles/, docs/, knowledge/ agent_structure) lives in Repo A (F:\JOB AI AGENT). Sophron intelligence (F:\Sophron, sibling repo) is completely separate. Zero cross-imports. Zero cross-writes. JobAgent index never embeds personal_intelligence; Sophron (B) intelligence never drives job loops. Bridge only via docs + SOPHRON_ROOT.
...
BATCH_JOB_EVALUATION (batch_question.json) or JOB_EVALUATION (pending_question.json) IPC task types.
...
[C32] Two-Stage Page Navigation Timeout Recovery (commit 60s + domcontentloaded 75s).
[C34] Radio Chip Option-Constrained Resolution (proficiency tier matching + options[0] retry).
