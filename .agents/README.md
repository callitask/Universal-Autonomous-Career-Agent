...
# Version: 4.0 | Updated: 2026-09-20 | Key change: Batch Architecture v2.0, SearchStateManager, ARM->BRAIN Batch IPC
...
`powershell
python core/continuous_career_agent.py --profile profiles/$TARGET_PROFILE --delay 90
`
- Runs in an infinite loop with a >=60-second cooldown between cycles (crucial for Batch IPC wait times).
...
`
- Polls pending_question.json (single card/chatbot IPC) every 2 seconds.
- Polls atch_question.json (discovery card batches) every 1 second.
...
`powershell
Get-Content -Tail 40 "<watcher_task_log_path>"
`
...
6. **CRITICAL: NEVER fabricate, invent, or exaggerate any degree, company, tool, technology, or metric not found in esume.md.**

#### Type 3: QUESTIONNAIRE (Screening Questions)
...
- **Open-ended / Descriptive fields:** Write a concise, factual paragraph derived strictly from esume.md.
...
1. **Create an Insight Card:** Write a Schema v2 JSON file to F:\JOB AI AGENT\Sophron\understanding_master\learned_insights\insight_YYYYMMDD_slug.json.
2. **Update Weekly Trend:** Append a single bullet point documenting the strategic pivot to F:\JOB AI AGENT\Sophron\understanding_master\macro_synthesis\current_week_trend.md.
...
**Remove:** domain_gated, low_score, 
egative_company_gated, elow_ctc_floor, 
o_native_apply, composite_gated
...
? **Never mix up** atch_answer.json (for batch cards) with pending_question.json (for tailoring/chatbot).  
...
2. **Load candidate profile** from profiles/$TARGET_PROFILE/candidate_config.json and esume.md into working memory — dynamically, with zero hardcoding.
