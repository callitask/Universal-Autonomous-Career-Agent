...
3. Key constraints you MUST internalize:
   ...
   - DIRECTIVE 4: Method signatures and contracts are immutable. evaluate_job_match() must respect the two-stage cognitive filter (negative gating -> domain title alignment -> incompatible vertical gating -> factual qualification).
   - DIRECTIVE 5: Naukri and LinkedIn handlers are strictly decoupled. Modals require container-isolated scrolling and React synthetic event dispatch for contenteditable elements.
   - DIRECTIVE 7: Canonical CSV tracker schema, deduplication logic, and status codes are strictly enforced.
...
4. After reading, confirm you understand by listing:
   ...
   - The method signature and return type for evaluate_job_match()
...
3. Key technical details you MUST internalize:
   ...
   - Section 3: Dual-Brain & IPC contracts:
     ...
     * Multi-session persistent ledger deduplication via ctx.load_processed_ledger() -> set and ctx.add_to_processed_ledger().
   ...
   - Section 6: Anti-detection timing parameters (30ms typing, 2500ms post-answer, domcontentloaded navigation).
   - Section 7 & 8: Failure mode matrix and guardrails H1-H6.

4. After reading, confirm you understand by listing:
   - The full pipeline execution sequence.
   - The IPC and ledger contract (which JSON files connect which scripts).
   - The ChatbotResolver control detection priority order and the React contenteditable injection protocol.
...
6. In 04_job_discovery.py, ensure ctx.load_processed_ledger() is handled safely as a set.
